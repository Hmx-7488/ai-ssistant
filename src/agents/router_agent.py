...
class RouterAgent:
    ...
    KEYWORDS: Dict[str, Iterable[str]] = {
        ...
        "点餐咨询": ["我要吃", "我要一份", "来一份", "点一份", "点餐", "辣吗", "忌口", "食材", "分量", "素食", "儿童餐", "营业", "地址", "wifi", "打包", "外卖", "会员", "优惠", "菜单", "所有菜", "全部菜", "能不能", "可以吗", "不要香菜", "有wifi", "停车", "发票", "生日", "有什么汤", "有什么饮品", "有什么甜品", "有什么配菜", "有什么小食", "有什么主食", "饮料", "喝的", "果汁", "茶水", "宝宝椅", "团购", "排队", "等位", "加饭", "续碗", "靠窗", "安静", "几人份", "几个人吃", "够吃", "放了什么", "有放", "热菜", "凉菜", "配菜"],
    ...
    def _classify_by_rules(self, user_input: str) -> str:
        ...
        # 0a. 分类查询（"有什么配菜"/"有主食吗"/"来点小吃"）→ 点餐咨询
        _CATEGORY_QUERIES = {
            "配菜": "配菜", "小吃": "小食", "小食": "小食",
            "热菜": "热菜", "凉菜": "凉菜", "凉拌": "凉菜",
        }
        for kw, _cat in _CATEGORY_QUERIES.items():
            if kw in normalized:
                return "点餐咨询"
